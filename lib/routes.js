...
module.exports = function(app) {
  ...
  app.use(function(req, res, next) {
      fs.exists(config.databaseFile, function(exists) {
        req.db = new sqlite3.Database(config.databaseFile, function(err) {
          if (err !== null)
            console.log('error' + err);
          if (!exists) {
            req.db.serialize(function() {
              req.db.run("CREATE TABLE files (id INTEGER PRIMARY KEY, userid INT, metadata TEXT, filepath TEXT)");
              req.db.run("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, password TEXT)");
              req.db.run("INSERT INTO users (name, password) VALUES('test', '123')");
            });
          }
          return next();
        });
